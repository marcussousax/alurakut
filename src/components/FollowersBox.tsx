...
export interface IFollowersBox {
    items: Record<string, boolean | string | number | undefined>[]
}

const FollowersBox = ({ items }: IFollowersBox) => {

    return (
        <FollowersBox.Wrapper>
            <h2 className="smallTitle">Meus seguidores ({items.length})</h2>
            <ul>
...
FollowersBox.Wrapper = styled(Box)`
  ul {
    display: grid;
    grid-gap: ${common.GAP};
